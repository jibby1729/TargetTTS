...
We trained with batch size 32, initial learning rate $10^{-3}$, Adam optimizer, hybrid loss $\alpha = 0.1$, power-law compression $p = 0.3$, and early stopping with patience 7. The learning rate is managed by ReduceLROnPlateau (factor 0.5, patience 3), which halves the learning rate whenever the validation loss stops improving for 3 consecutive epochs.
